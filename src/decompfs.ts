...
export class decfs implements vscode.FileSystemProvider {
    ...
    addDecompTarget(file: vscode.Uri): void {
        ...
        const decompDir = vscode.Uri.parse("decfs:/" + filename + "/");
        const filePath = vscode.Uri.joinPath(decompDir, filename);
        const contents = readFileSync(file.fsPath);
        console.log(
            "Creating: ",
            filename,
            "in",
        ...
        try {
            this.createDirectory(decompDir);
        } catch (e) {
            console.log("Got filesystem error: ", e);
            throw e;
        }
